...
import { parseModbusCsvConfig, formatElementRange, formatModbusAddrRange, formatDeviceAddrRange, extractNsp, sortModbusTags } from "../utils/csvTagParser";

interface CsvTagDatabaseViewProps {
  tags: ModbusTag[];
...
export default function CsvTagDatabaseView({
  tags,
  activeServerId = "ALL",
  servers = [],
  onServerSelect,
  ...
  const activeServerObj = servers.find((s) => s.id === activeServerId);
  const isRtuServer = activeServerObj?.mode === "rtu" || (activeServerId && (activeServerId.startsWith("RTU") || activeServerId.startsWith("COM")));
  ...
  const filteredTags = useMemo(() => {
    ...
    return sortModbusTags(list);
  }, [tags, searchQuery, selectedBank, selectedUpdateMode, activeServerId]);

  return (
    <div className="flex flex-col h-full bg-[#111] overflow-hidden">
      {/* Sub-Header Toolbar */}
      <div className="sticky top-0 z-30 border-b border-neutral-800 bg-neutral-900/95 backdrop-blur p-3 flex flex-wrap items-center justify-between gap-3 shadow-md shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5"
          ...
          <button
            ...
            title="Restore the default factory template tags (8 sample CENTUM VP tags)"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#0a0a0a]">
        {activeSubTab === "import" ? (
          <div className="space-y-4 max-w-4xl mx-auto py-2">
            ...
            <div className="border-2 border-dashed border-neutral-800 hover:border-indigo-500/50 bg-[#111] rounded-xl p-6 text-center transition-all">
              <Upload className="w-10 h-10 text-indigo-400 mx-auto mb-3 opacity-80" />
              <h4 className="text-sm font-semibold text-white mb-1">
                Upload Yokogawa Modbus CSV File
              </h4>
              ...
              <input
                type="file"
                accept=".csv,.txt"
                id="csv-file-input-tab"
                onChange={handleFileUpload}
                className="hidden"
              />
              <label
                htmlFor="csv-file-input-tab"
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-[#111] border border-neutral-800 rounded-xl p-3 font-mono text-xs text-neutral-300 outline-none focus:border-indigo-500 leading-relaxed"
              />
            </div>
          ...
          <div className="space-y-4">
            {/* Stats Summary Bar */}
            {parseStats && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-[#111] border border-neutral-800 p-3 rounded-xl">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-white font-mono">{parseStats.buffersCount}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-white font-mono">{parseStats.blocksCount}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-bold text-white font-mono">{parseStats.tagsCount}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2.5">
                  <div className="p-1.5 bg-neutral-800 rounded-lg text-neutral-400">
                    <Database className="w-4 h-4" />
                  </div>
                  <div>
                    <span className="text-[10px] text-neutral-400 uppercase font-semibold block">Gap Unallocated</span>
                    <span className="text-xs font-bold text-white font-mono">{parseStats.gapWords} words</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
              <div className="relative w-full md:w-72">
                ...
                <input
                  ...
                  placeholder="Search Label, Comment, or Address..."
                  className="w-full bg-[#111] border border-neutral-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-white placeholder-neutral-500 outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <select
                  ...
                  className="bg-[#111] border border-neutral-800 rounded-lg px-2.5 py-1.5 text-xs text-neutral-300 outline-none focus:border-indigo-500"
                >
            ...
            {/* Tag Table */}
            <div className="overflow-auto border border-neutral-800 rounded-xl bg-[#111] max-h-[calc(100vh-280px)] min-h-[350px] relative">
              <table className="w-full border-separate border-spacing-0 text-left min-w-[750px]">
                <thead>
                  <tr className="text-neutral-300 text-[11px] uppercase font-semibold">
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">CENTUM Tag Label</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">{serverColHeader}</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Hardware Addr</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Update Mode & MB Addr</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Data Type</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Current Value</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Sim Pattern</th>
                    <th className="px-4 py-2.5 sticky top-0 z-30 bg-[#161616] border-b border-neutral-700 shadow-sm">Service Description</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-800/60 text-xs font-sans">
                  {filteredTags.map((t, idx) => (
                    <tr
                      key={t.id ? `${t.id}_${idx}` : `tag_${t.serverId || ''}_${t.modbusAddress}_${idx}`}
                      className="even:bg-white/[0.02] odd:bg-transparent hover:bg-white/[0.04] transition-colors"
                    >
                      ...
                      <td className="px-4 py-2 font-mono whitespace-nowrap">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold font-mono border ${
                          ...
                          {(t.isRtu || !t.ipAddress)
                            ? (t.nsp || extractNsp(t.programName, t.port))
                            : (t.ipAddress || t.serverId?.split(":")[0] || "127.0.0.1")}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-2 font-mono text-emerald-400 font-bold whitespace-nowrap">
                        <input
                          type="number"
                          value={t.value}
                          onChange={(e) => {
                            ...
                            if (onUpdateTag) {
                              ...
                              const updated = tags.map((x) =>
                                x.id === t.id ? updatedTag : x
                              );
                              onUpdateTags(updated);
                            }
                          }}
                      ...
                      <td className="px-4 py-2 text-neutral-400 max-w-xs truncate" title={t.comment}>
                        {t.comment}
                      </td>
                    </tr>
                  ))}
                  {filteredTags.length === 0 && (
                    <tr>
                      <td colSpan={8} className="p-12 text-center text-neutral-500 text-xs">
                        {tags.length === 0 ? (
                          <div className="flex flex-col items-center justify-center gap-3">
                            <p className="text-neutral-400 font-medium">Tag Database is currently empty (0 tags).</p>
                            <p className="text-neutral-500 max-w-sm">
                              Upload a Yokogawa Modbus CSV file above, paste raw configuration text, or restore the baseline factory default tags.
                            </p>
                            <button
                              onClick={onFactoryReset}
