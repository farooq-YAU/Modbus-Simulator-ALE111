...
export default function App() {
  ...
  const importCsv = (source = csv) => { const result = parseCsv(source); if (result.errors.length || !result.tags.length) { setNotice(result.errors.join(" ") || "No Modbus tags found."); return; } updateTags(result.tags); setNotice(`Imported ${result.tags.length} tags.`); };
  const browseCsv = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const source = await file.text(); setCsv(source); importCsv(source); event.target.value = ""; };
  ...
  return <div className="shell">
    ...
    <main><aside><div className="panel-title">Register banks</div>{banks.map(item => <button className={bank === item.id ? "nav active" : "nav"} key={item.id} onClick={() => setBank(item.id)}>{item.label}<b>{tags.filter(tag => tag.regBank === item.id).length}</b></button>)}<div className="panel-title">CSV configuration</div><textarea value={csv} onChange={event => setCsv(event.target.value)} placeholder="Paste CENTUM VP CSV here..." /><div className="csv-actions"><button onClick={() => importCsv()}>Import CSV</button><label className="file-button">Browse CSV<input className="file-input" type="file" accept=".csv,text/csv" onChange={browseCsv} /></label></div>{notice && <p className="notice">{notice}</p>}</aside>
